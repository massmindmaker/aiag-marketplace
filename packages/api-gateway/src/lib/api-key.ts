import { createHash, randomBytes } from 'node:crypto';

/**
 * API key format: `sk_aiag_<env>_<22-char-base64url>`
 * - env = 'live' | 'test'
 * - stored as SHA-256 hex in `gateway_api_keys.key_hash`
 * - key_prefix = first 20 chars (stable lookup / display)
 */
export const KEY_PREFIX_REGEX = /^sk_aiag_(live|test)_[A-Za-z0-9_-]{20,}$/;

export function generateApiKey(env: 'live' | 'test' = 'live'): {
  key: string;
  hash: string;
  prefix: string;
} {
  const raw = randomBytes(24).toString('base64url');
  const key = `sk_aiag_${env}_${raw}`;
  return {
    key,
    hash: hashKey(key),
    prefix: key.slice(0, 20),
  };
}

export function hashKey(key: string): string {
  return createHash('sha256').update(key).digest('hex');
}

export function parseBearer(authHeader: string | null | undefined): string | null {
  if (!authHeader) return null;
  const m = /^Bearer\s+(\S+)$/.exec(authHeader.trim());
  return m ? m[1]! : null;
}
